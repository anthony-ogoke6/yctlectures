...
class OwnerDepartmentEditMixin(OwnerDepartmentMixin, OwnerEditMixin):
    fields = ['school', 'department_access', 'level', 'name', 'amount', 'overview', 'photo', 'general']
    success_url = reverse_lazy('learn:manage_department_list')
    template_name = 'learn/form.html'
...
def add_assignment(request, topic_id, slug, course_id):
	...
	return render(request, 'learn/content/form3.html', context)
...
def delete_student(request):
    ...
    if request.is_ajax():
        html = render_to_string('learn/stud.html', context, request=request)
