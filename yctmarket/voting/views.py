...
from .forms import ModuleFormSet, CreateForm, ModuleFormSet1, ModuleFormSet2, DepartmentForm, VoteForm, VoteFormStudents
...
class OwnerDepartmentEditMixin(OwnerDepartmentMixin, OwnerEditMixin):
    fields = ['school', 'department_access', 'level', 'name', 'photo', 'general']
    success_url = reverse_lazy('voting:manage_department_list')
    template_name = 'voting/form.html'
...
class DepartmentCreateView(OwnerDepartmentEditMixin, CreateView):
	...
	def form_valid(self, form):
	    ...
	    obj.save()
	    course = Department.objects.get(school=school, instructor=self.request.user, slug=obj.slug)
	    print(course)
	    for g in allow2:
...
class DepartmentCourseUpdateView(TemplateResponseMixin, LoginRequiredMixin, View):
	...
	def post(self, request, *args, **kwargs):
		...
		if formset.is_valid():
		    for form in formset:
		        ...
		        course_update.save()
		    messages.success(request, "Year has been updated successfully.", extra_tags='success')
		    return redirect('voting:manage_department_list')
		return self.render_to_response({'course': self.course, 'formset': formset})


class ContentCreateUpdateView(TemplateResponseMixin, LoginRequiredMixin, View):
	topic = None
	model = None
	obj = None
...
class CourseContentListView(TemplateResponseMixin, LoginRequiredMixin, View):
...
class DepartmentDetailView(DetailView):
	...
	def get_context_data(self, **kwargs):
		...
		context['profileform'] = VoteFormStudents()
...
def add_assignment(request, topic_id, slug, course_id):
	...
	if request.method == 'POST':
		...
		if formset.is_valid():
			...
			messages.success(request, "Position has been added successfully.", extra_tags='success')
		...
		if formset.is_valid():
			for form in formset.forms:
				...
				for i in formset.deleted_forms:
				    ...
				    print(i)
				assignment.save()
				messages.success(request, "Position has been edited successfully.", extra_tags='success')
			return redirect('voting:topic_content_list', topic_id=topic_id)
	else:
...
def delete_department_access(request):
    if request.is_ajax and request.method == 'POST':
        ...
        department = Department.objects.get(school=obj_name_of_department_for_this_course, slug=name_of_course_from_ajax)
    ...
    if request.is_ajax():
        html = render_to_string('voting/dept.html', context, request=request)
        return JsonResponse({'form': html})
