...
def store(request, tag_slug=None):
    ...
    for shop in stores:
        ...
        #check when post was created
        post_created = shop.created.date()
...
def store_detail(request, id=None, slug=None, category=None):
    post_increament = get_object_or_404(Stores, id=id, slug=slug, category=category)
    post_increament.view_count +=1
    post_increament.save()
    related = []
    store = get_object_or_404(Stores, id=id, slug=slug, category=category)
...
def shop_detail(request, id, slug, title):
    post_increament = get_object_or_404(Product, id=id, slug=slug, title=title)
    post_increament.view_count +=1
    post_increament.save()
    related = []
    post = get_object_or_404(Product, id=id, slug=slug, title=title)
    url = f"https://yctmarket.pythonanywhere.com{post.logo.url}"
    q = post.title
    author = post.author
    amount = post.amount
    title = post.title
