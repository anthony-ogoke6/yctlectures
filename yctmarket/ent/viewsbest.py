...
from .forms import *
from .forms import EmailFormForMeetUp, EmailFormForPayment, InfiniteScrollForm, PostCreateForm, PostCreateFormForFree, UserLoginForm, UserRegistrationForm, ReRequestActivationForm, UserEditForm, ProfileEditForm
import requests
import uuid
...
def article_list(request, tag_slug=None):
    ...
    ads = AdvertImages.objects.all()[:3]
    a = ads[0]
    b = ads[1]
    c = ads[2]
...
@login_required
def profile(request):
    return render(request, 'ent/profile.html', {'user_form': user_form, 'profile_form': profile_form})
