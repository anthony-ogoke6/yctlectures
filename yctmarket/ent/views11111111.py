...
from .forms import *
from .forms import EmailFormForMeetUp, EmailFormForPayment, InfiniteScrollForm, PostCreateForm, ProfileFormStudents, ProfileFormTutors, PostCreateFormForFree, UserLoginForm, ProfileForm1, UserRegistrationForm, ReRequestActivationForm, CommentForm, ProfileForm, ProfileFormTutors, UserEditForm, ProfileEditForm,  ShopFreeForOneMonth, ShopPaymentSixMonth, UploadProductForFree, ShopPaymentOneYearMonth
import requests
import uuid
...
def signup_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            subject = f"Student named {username} just registered at yctmarket"
...
def signup_tutor_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid() and profile_form.is_valid():
            ...
            user_phone.save()
            #Profile.objects.create(user=new_user, prefix=prefix)
            subject = f"Tutor named {username} just registered at yctmarket"
            message = '%s %s ' %(email, phone_number)
            emailFrom = [settings.EMAIL_HOST_USER]
            emailTo = [settings.EMAIL_HOST_USER]
            send_mail(subject, message, emailFrom, emailTo, fail_silently=True )
...
def signup_students_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid() and profile_form.is_valid() and profile_form_students.is_valid():
            new_user = form.save(commit=False)
            #new_profile = profile_form.save(commit=False)
            email = form.cleaned_data['email']
            dob = request.POST['dob']
            ...
            my_group.user_set.add(user)
            saved_user = User.objects.get(username=username)
            user_phone = PhoneNumber(user=saved_user, phone_number=phone_number, matric_number=matric_number, department=department)
            user_phone.save()
            #Profile.objects.create(user=new_user, dob=dob, photo=photo)
            subject = f"Customer named {username} just registered at yctmarket"
            message = '%s %s ' %(email, phone_number)
            emailFrom = [settings.EMAIL_HOST_USER]
            emailTo = [settings.EMAIL_HOST_USER]
            send_mail(subject, message, emailFrom, emailTo, fail_silently=True )
...
def edit(request):
    ...
    if request.method == 'POST':
        ...
        if request.user.groups.filter(name='Instructors'):
            profile_form1 = ProfileFormTutors(instance=phonenumber, data=request.POST)

        else:
            profile_form1 = ProfileFormStudents(instance=phonenumber, data=request.POST)
            #profile_form1 = ProfileFormTutors(instance=phonenumber, data=request.POST)
        user_form = UserEditForm(instance=request.user, data=request.POST)
        profile_form = ProfileEditForm(instance=request.user.profile, data=request.POST, files=request.FILES)

        if user_form.is_valid() and profile_form.is_valid() and profile_form1.is_valid():
            #profile_form.save()
            #user_form.save()
            p1 = profile_form.save(commit=False)
            p2 = profile_form1.save(commit=False)
            print('p1')
            print(p1)
            print(p2)
            p1.user = user
        ...
        #photo = None
    return render(request, 'ent/edit.html', {'shop_owned_by_user': shop_owned_by_user, 'user_form': user_form, 'profile_form': profile_form, 'profile_form1': profile_form1, 'user_posts':user_posts, 'profile':profile, 'shop_owners_list':shop_owners_list})



@login_required(login_url="user_login")
def upload_product1(request, store_id, store_slug):
    store_id = store_id
    store_slug = store_slug
    store = get_object_or_404(Stores, id=store_id, slug=store_slug)
    user = request.user
    if request.method == 'POST':
        form = UploadProductForFree(instance=request.user, data=request.POST, files=request.FILES)
        if form.is_valid():
            r = uuid.uuid4()
            reference = str(r)
            post = form.save(commit=False)
...
def post_delete(request, id, slug):
    ...
    if request.user != post.author:
        raise Http404()
    post.delete()
    messages.error(request, 'Post has been successfully deleted')
    return redirect('ent:article_list')
