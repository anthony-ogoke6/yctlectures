...
def assignment(request, id, slug, topic_id):
	...
	article1 = []
	stores = Face.published.all()
	news_search = request.GET.get('search')
	if news_search:
	    stores = Face.published.filter(
	        Q(storename__icontains=news_search)|
	        Q(description__icontains=news_search)).distinct()
